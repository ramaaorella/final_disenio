...
- **_Gateway de mensajería:_** Se descarta en principio el procesamiento de combinado de mensajes entrantes y salientes. Deberá encargarse de homogeneizar el input recibido por cada sistema (considerando la integración con APIs futuras) para poder procesar los mensajes correctamente en el Core; y también el output a enviarle a cada sistema. Además, deberá priorizar los mensajes de aquellos usuarios que se encuentren en línea en el portal (para mayor fluidez), y deberá ser capaz de garantizar la entrega de mensajes (en la comunicación con el servicio de integración especial del Core y en la comunicación con los sistemas con los que interopera), considerando que los servicios pueden estar no disponibles o que incluso el servidor de RabbitMQ puede detenerse o reiniciarse. Debe también llevar cuenta de la trazabilidad de los mensajes.
- **_Core (en forma abstracta):_** Deberá mantener la trazabilidad de los mensajes, el conjunto de usuarios en línea (para darles prioridad) y demás datos necesarios para el portal. La comunicación entre servicios no debe generar una sobrecarga adicional de gran impacto; sobre todo la comunicación con el Gateway de mensajería debe ser lo más fluida posible.
- **_Servicio de integración especial:_** Además de articular con el Gateway de mensajería para recibir y enviar mensajes, deberá comunicarse con un/os servicio/s de negocio/s para persistir todos los mensajes (asegurándose de no impactar en gran medida en el rendimiento del sistema) y procesarlos.

<ins>Concerns de la arquitectura inicial:</ins>

- La comunicación entre el Gateway de mensajería y el Core (mediante su servicio de integración especial) puede actuar de cuello de botella del sistema, al igual que la persistencia de mensajes y demás datos, que requiere ser lo suficientemente adecuada para manejar la posible gran cantidad de usuarios y mensajes entrantes y salientes, además de la velocidad de procesamiento requerida para lograr la fluidez necesaria para el objetivo de negocio. Debe considerarse también que toda comunicación puede pasar a ser potencialmente una comunicación con componentes/servicios desplegados en la nube.
...
- La pérdida de mensajes en el Gateway de mensajería puede ser irrecuperable (sobre todo para los mensajes entrantes).
